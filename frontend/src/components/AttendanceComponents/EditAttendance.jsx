...
export function EditAttendance() {
  ...
  const [attendanceFetched, setAttendanceFetched] = useState(false); // State to track if attendance is fetched
  const navigate = useNavigate();

  const handleFetchAttendance = () => {
    // Fetch existing attendance data for the selected date
    // For simplicity, we're using initialData here
    const updatedData = initialData.map((item) => ({
      ...item,
      date,
    }));
    setAttendanceData(updatedData);
    setAttendanceFetched(true); // Set attendanceFetched to true after fetching
  };
  ...
  return (
    <div className="flex flex-col">
      ...
      <ArrowLabel label="Edit Attendance" location="/home/attendance" />
          ...
          <div className="my-4 flex justify-center">
            <button
              className={`bg-purple-500 py-2 px-6 rounded-lg ${
                !attendanceFetched && "opacity-50 cursor-not-allowed"
              }`}
              onClick={handleSave}
            >
              Save
